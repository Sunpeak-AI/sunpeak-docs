---
title: "Runtime APIs"
description: "Build once, run on ChatGPT, Gemini, and Claude"
---

## Overview

sunpeak provides a multi-platform runtime abstraction layer that lets you build
widgets once and potentially deploy them across different AI chat platforms like
ChatGPT, Gemini, and Claude.

## Why Multi-Platform?

<CardGroup cols={2}>
  <Card title="Write Once, Deploy Everywhere" icon="recycle">
    Build your widget logic once, run it on multiple platforms without rewriting
  </Card>

  <Card title="Consistent Developer Experience" icon="code">
    Same React hooks and patterns across all platforms
  </Card>

  <Card title="Future-Proof" icon="shield">
    Automatically support new platforms as they emerge
  </Card>

  <Card title="Easy Migration" icon="arrow-right-arrow-left">
    Switch platforms without major code changes
  </Card>
</CardGroup>

## React Hooks

sunpeak provides platform-agnostic React hooks for common runtime features:

```tsx
import { useTheme, useDisplayMode, useWidgetState } from 'sunpeak';

function MyWidget() {
  const theme = useTheme();                    // 'light' | 'dark'
  const displayMode = useDisplayMode();        // 'mobile' | 'tablet' | 'desktop'
  const [data, setData] = useWidgetState('key', defaultValue);

  return <div className={theme === 'dark' ? 'dark-mode' : 'light-mode'}>...</div>;
}
```

## Dive Deeper

<Card title="Hooks" icon="code" href="/api-reference/hooks/use-theme">
  All hooks provided by sunpeak.
</Card>
