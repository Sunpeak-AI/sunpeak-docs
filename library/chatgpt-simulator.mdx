...
<Tabs>
  <Tab title="Framework (JSON)">
    In the sunpeak framework, simulations are **JSON files** co-located with their resource.

    Simulations live in the same folder as their resource (`src/resources/{name}/`):
    ...
    The resource component (`example-resource.tsx`) and metadata (`example-resource.json`) are automatically linked since they live in the same folder.
