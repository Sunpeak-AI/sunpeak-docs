...
- `src/simulations/` - Directory containing simulation JSON files.
    - Files in this directory matching `*-simulation.json` are
    automatically discovered by the dev server and MCP server.
    - Simulations are linked to resources by filename prefix
    (e.g., `albums-show-simulation.json` links to `albums-resource.tsx`).
