...
2. Reads metadata from `*.meta.json` files (name, title, description, mimeType)
3. Uploads each resource to the Sunpeak repository
4. Sunpeak assigns a unique URI to each resource to invalidate the ChatGPT cache
5. Optionally assigns a tag for version management

<Info>
**Resource names must be unique within a repository.** If you push a resource with a name that already exists, it will update the existing resource. The resource name is derived from the `name` field in your `.meta.json` file.
</Info>
...
├── search.meta.json    # Metadata for search resource
├── dashboard.js        # Resource: dashboard
└── dashboard.meta.json # Metadata for dashboard resource
```

Each resource needs a corresponding `.meta.json` file containing metadata like name, title, and description. URIs are assigned by Sunpeak when you push.
