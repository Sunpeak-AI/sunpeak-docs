...
For example:
- `search-resource.tsx` + `search-resource.json` → `dist/search.js` + `dist/search.json`
- `calendar-resource.tsx` + `calendar-resource.json` → `dist/calendar.js` + `dist/calendar.json`
