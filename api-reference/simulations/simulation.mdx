...
<ResponseField name="resource" type="Resource" required>
  Official MCP SDK Resource type used in the ListResources response. Defines the resource name, description, and other metadata.

  **Required**: Resource metadata must be defined in a separate `-resource.json` file in the `resources`
  directory. You can either duplicate those values here or import the JSON file and put it into your simulation.
  This keeps metadata server-safe (no React/CSS imports), allows the build to export it, and enables deployment.

  **Key properties**:
  - `name`: Name of the resource (machine). **Must be unique within a repository.**
...
The simulation imports the resource metadata from the JSON file:
