...
<Tabs>
  <Tab title="Framework (JSON)">
    In the sunpeak framework, simulations are **JSON files** co-located with their resource. The resource component and metadata are automatically linked based on directory structure.

    ## File Naming Convention

    Simulation files live alongside their resource in the same folder:

    ```
    src/resources/{name}/{name}-{scenario}-simulation.json
    ```

    For example:
    - `src/resources/albums/albums-show-simulation.json`
    - `src/resources/review/review-diff-simulation.json`
    - `src/resources/review/review-post-simulation.json`

    Each resource folder contains all related files:
    - **Component**: `src/resources/{name}/{name}-resource.tsx`
    - **Metadata**: `src/resources/{name}/{name}-resource.json`
    - **Simulations**: `src/resources/{name}/{name}-{scenario}-simulation.json`
    ...
    ```json
    // src/resources/example/example-show-simulation.json
    {
    ...
    1. **Discovery**: Scans each resource folder for `*-simulation.json` files
    2. **Linking**: Simulations are automatically linked to their co-located resource
    ...
    src/resources/albums/
      albums-resource.tsx              # Component
      albums-resource.json             # Metadata
