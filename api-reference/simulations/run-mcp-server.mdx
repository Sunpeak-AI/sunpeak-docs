...
When a tool is called, it returns the `toolCall.structuredContent` from that simulation,
which your widget can access via `useWidgetProps()`. It also returns the `toolCall._meta`,
which can be accessed through `useToolResponseMetadata()`.
...
For production, create your own MCP server that returns your published widget URL as a resource.
